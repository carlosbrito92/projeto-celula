...
- [x] Três extensões à spec de privacidade, motivadas por teste de campo real (célula ao vivo, Artista Impostor com papel e caneta): banco de categorias de palavras (fecha vazamento — quem configura nunca vê a palavra, só a categoria), nomes de participantes persistentes entre sessões (localStorage), reordenação por arraste da lista de nomes (`@dnd-kit/sortable`, escolhido por suporte real a touch — HTML5 drag nativo não funciona bem em telas de toque). Capacidade de categoria implementada e testada; conteúdo real das listas de palavras ainda pendente (Carlos vai fornecer, não foi inventado) — `content/quebra-gelos/artista-impostor.json` continua sem `categorias` até isso chegar.
