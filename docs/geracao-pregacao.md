...
```json
{
  ...
  "mapa_pontos": [
    ...
    { "id": "ponto-2", "numero": "02", "titulo": "string curto", "pendente": "opcional — booleano, true quando o ponto foi anunciado mas não desenvolvido (ex: sem tempo hábil). Sem secoes[] correspondente; índice renderiza esmaecido e não-clicável." }
  ],
  "secoes": [
    {
      "id": "ponto-1",
...
- **`mapa_pontos[].pendente`** — booleano opcional, `true` quando o pregador **de fato** anunciou um ponto e não chegou a desenvolvê-lo (ex: tempo do culto esgotado) — não quando a transcrição está ruidosa e a contagem de pontos fica incerta. Um ponto `pendente` **não tem `secoes[]` correspondente** (não há conteúdo pra seção nenhuma); a plataforma renderiza o card do índice esmaecido e não-clicável em vez de tentar rolar pra uma seção inexistente. Registrar também em `nota_lacuna[]` o motivo do ponto não ter sido desenvolvido, pra contexto de quem lê depois.
  - **Falso positivo na calibração de "O Véu É Removido" (Pastor Ian), corrigido em 2026-08-12**: a geração inicial (Modo A, transcrição com ruído) leu "6 verdades" anunciadas e só 5 desenvolvidas, marcando uma 6ª entrada como `pendente`. Anotação oficial de um membro presente no culto confirmou depois: a mensagem sempre teve **5 verdades completas** — o que soou como "6ª verdade" na transcrição era na verdade o início da seção separada "Conclusão e Apelo" (2 Coríntios 3:18). Corrigido removendo a entrada `pendente` e adicionando `conclusao` como seção normal (numero `"✦"`), não como ponto numerado. Lição: `pendente` é pra lacuna real da pregação, não pra incerteza de transcrição — ruído severo (Modo A) deve virar `nota_lacuna` com ressalva (ver abaixo), nunca virar direto um `pendente` sem confirmação externa.
