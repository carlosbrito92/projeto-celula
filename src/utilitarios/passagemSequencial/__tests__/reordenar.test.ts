...
describe('moverPorIndice', () => {
  ...
  it('índice fora dos limites não muda a lista (defensivo — não deveria acontecer via drag real)', () => {
    const lista = ['Ana', 'Beto'];
    expect(moverPorIndice(lista, '0', '5')).toBe(lista);
    expect(moverPorIndice(lista, '-1', '0')).toBe(lista);
  });

  it('id não numérico não muda a lista', () => {
    const lista = ['Ana', 'Beto'];
    expect(moverPorIndice(lista, 'x', '0')).toBe(lista);
  });
});
